...
public class SlottedBlock
{
    ...
    */ 
    public void dumpBlock()
    {
    	...
    	System.out.println("Available space in bytes: " + getAvailableSpace() + " ." );
    	System.out.println("---------The header slot array is printed---------");
    	for (int i = 0; i < lenHeader; i++) {
    		System.out.println("In index " + Integer.toString(i) + " the entry is: " + Integer.toString(intBuffer.get(i)));
    	}
    	int entryLength = 0;
    	int entryIndex = 0;
    	System.out.println("---------The records are printed---------");
    ...
    */
    public RID insertRecord(byte[] record)
    {
        ...
        if (emptySlot == true) {
        	System.out.println("Yeah there is empty slot in header!");
        }
        if ((emptySlot == false) && (record.length > getAvailableSpace() - 8)) { //available space with no extra header space needed
        	System.out.println("The block is full. Go to the next block.");
    ...
    */
    ...
    {
    	if (rid == null){
    		System.out.println("The given rid is not deleted (null pointer).");
    		return false;
    	}
    	if (!(rid.blockId == intBuffer.get(1) && intBuffer.get(rid.slotNum) != 0)){
    		System.out.println("The given rid is deleted.");
    		return false;
    	}
    	lenHeader();
        ...
        lenHeader();
        System.out.println("The rid entry is deleted.");
        return true;
    }
